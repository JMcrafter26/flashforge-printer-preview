...
# Author: JMcrafter26
# URL: https://gist.github.com/JMcrafter26/238296b25dc22d4e309496f94679ba05
...
# cd into api/ directory if not already in it
if not os.getcwd().endswith('api'):
    os.chdir('api')
...
with open(os.path.join(programRelease, f'backend.min.py'), 'w') as file:
    file.write(minified)

# write the original file to main.py
with open(os.path.join(programRelease, f'backend.py'), 'w') as file:
    file.write(comment + '\n\n\n' + ''.join(lines))


os.remove(os.path.join(programReleaseRaw, 'tempPackFile.py'))
# remove program-raw directory
shutil.rmtree(programReleaseRaw)

# from ../ copy index.html to release/program/
if os.path.exists('../index.html'):
    shutil.copy2('../index.html', programRelease)
...
os.chdir(programRelease)
# build the program using pyinstaller, set name to backend
os.system(f'pyinstaller --onefile backend.min.py --name backend --clean')

# move the dist folder to the release folder
shutil.move('dist', os.path.join(programRelease, 'dist'))

# remove the build and __pycache__ folders
shutil.rmtree('build')

# move dist/backend.exe to release/backend.exe
shutil.move(os.path.join(programRelease, 'dist', 'backend.exe'), os.path.join(programRelease, 'backend.exe'))

# remove the dist folder
shutil.rmtree(os.path.join(programRelease, 'dist'))
