...
class RequestHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        # if no command is set, show the available commands
        elif command is None:
            self.send_response(200)
            self.send_header('Content-type', 'application/json')
            self.end_headers()
            self.wfile.write(json.dumps({'error': 'No command provided', 'commands': list(commands.keys()), 'example': 'http://localhost:8899/info?ip=0.0.0.0[&port=8899]'}).encode())
        else:
