...
class RequestHandler(BaseHTTPRequestHandler):
	def do_GET(A):
		F='Invalid IP or port';D='application/json';E='Content-type';B='error';C=get_command(A.path);G,H=get_ip_and_port(A.path);print(f"Command: {C} | IP: {G} | Port: {H}")
		if C in commands:
			A.send_response(200);A.send_header(E,D);A.end_headers();I=getResponse(C,G,H)
			if I is _A:A.wfile.write(json.dumps({B:F}).encode())
			else:
				try:A.wfile.write(json.dumps(I).encode())
				except:A.wfile.write(json.dumps({B:F}).encode())
		elif C is _A:A.send_response(200);A.send_header(E,D);A.end_headers();A.wfile.write(json.dumps({B:'No command provided','commands':list(commands.keys()),'example':'http://localhost:8899/info?ip=0.0.0.0[&port=8899]'}).encode())
		else:A.send_response(404);A.send_header(E,D);A.end_headers();A.wfile.write(json.dumps({B:'Command not found'}).encode())
def run():A='localhost',8899;B=HTTPServer(A,RequestHandler);print('Starting http server on http://localhost:8899');B.serve_forever()
run()